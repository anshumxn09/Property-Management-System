...
import mySQLConnection as database, adminMainPage, rentSearhResult
...
class RentSearch:
    def __init__(self):
        ...
        def fetch():
            ...
            try:
                yup = int(getID)
                database.myQuery.execute(f"select * from myrent where r_id={yup}")
                if database.myQuery:
        ...
        search = Label(headerLabel, text="Enter Rent's ID : ", font=("Times New Roman", 16, "bold"))
