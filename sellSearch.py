...
import mySQLConnection as database, adminMainPage, sellSearchResult
...
class SellSearch:
    def __init__(self):
        ...
        def fetch():
            ...
            try:
                yup = int(getID)
                database.myQuery.execute(f"select * from mysell where s_id={yup}")
                if database.myQuery:
        ...
        search = Label(headerLabel, text="Enter Sell's ID : ", font=("Times New Roman", 16, "bold"))
        ...
        searchButton = Button(headerLabel, command=fetch, text="⇐", font=("Times New Roman", 16, "bold"))
