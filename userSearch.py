...
import mySQLConnection as database, adminMainPage, userSearchResult
...
class UserSearch:
    def __init__(self):
        ...
        def fetch():
            ...
            try:
                yup = int(getID)
                database.myQuery.execute(f"select * from myuser where user_id={yup}")
                if database.myQuery:
        ...
        search = Label(headerLabel, text="Enter User's ID : ", font=("Times New Roman", 16, "bold"))
